...
class DiffTexture(nn.Module):
    ...
    def img_show(self, height=512, width=512):
        img_tensor = self.render_img(height, width)[:, 0:3, :, :] #if latents, maintain the first 3 components
    ...
    def img_save(self, save_path, height=512, width=512, rgb=True):
        ...
        if self.is_latent and rgb:
            img_tensor = self.render_img(height=self.height, width=self.width) # pay attention to this line!!(and last line)
